...
from .cli import parse_args_to_config
from .log_utils import log, time_block
from .geom_boozer import make_grids, accum_RZnu_derivs, push_off_surface, cyl_xyz_from_RphiZ
from .coils_utils import gamma_from_xyz_columns, circular_guess, fit_curves_from_gamma, build_coils
from .opt_runner import run_optimization
from .trace_utils import initial_conditions_from_vmec, trace_fieldlines
from .plot_helpers import surface_trace_from_RZ_phi, tubes_mesh3d_from_gammas, add_polyline_trajs, npf

def main(argv=None):
    args, cfg = parse_args_to_config(argv)
    ...
    # Accumulate R,Z,nu and derivatives
    ...
    # Cylindrical→Cartesian for coil centerlines
    phi = varphi - nu
    ...
    # Constraint caps from Booz-XFORM fit
    ...
    coils_orig = [np.column_stack([npf(X)[:,i], npf(Y)[:,i], npf(Z)[:,i]])[::cfg.decimate]
                  for i in range(cfg.ncoils)]
    data.append(tubes_mesh3d_from_gammas(coils_orig,
    ...
    data.append(tubes_mesh3d_from_gammas(coils_opt,
                                         radius=cfg.tube_radius,
                                         n_theta=cfg.tube_theta,
                                         color="#CD9B3F",
                                         opacity=cfg.tube_opacity))
